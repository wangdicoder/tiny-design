...
function deriveSemanticTokensLight(
  baseKey: string,
  hex: string
...
function deriveBgTokensLight(hex: string): Record<string, string> {
  const { h, s, b } = hexToHsb(hex);
  const spotlight = deriveHex(h, s, Math.max(0, b - 4));
  return buildBgMap(hex, spotlight);
}

function deriveTextTokensLight(hex: string): Record<string, string> {
  return {
    'color-text': hex,
    'color-text-secondary': hex.startsWith('rgba') ? hex : adjustOpacity(hex, 0.65),
    'color-text-tertiary': hex.startsWith('rgba') ? hex : adjustOpacity(hex, 0.45),
    'color-text-quaternary': hex.startsWith('rgba') ? hex : adjustOpacity(hex, 0.25),
  };
}

function deriveBorderTokensLight(hex: string): Record<string, string> {
  const { h, s, b } = hexToHsb(hex);
  return {
    'color-border': hex,
...
function derivePrimaryTokensDark(hex: string): Record<string, string> {
  ...
  // Dark primary bg: very dark, low saturation
  const bg = deriveHex(h, Math.min(40, s), 15);
  const border = deriveHex(h, Math.min(60, s), 35);
...
function deriveSemanticTokensDark(
  baseKey: string,
  hex: string
): Record<string, string> {
  const { h, s, b } = hexToHsb(hex);
  ...
  // Dark: bg is very dark tinted, border is medium dark, text is lighter
  ...
  if (baseKey === 'color-danger') {
    tokens['color-danger-hover'] = deriveHex(h, Math.max(0, s - 10), Math.min(100, b + 12));
  }

  return tokens;
}

function deriveBgTokensDark(hex: string): Record<string, string> {
  const { h, s, b } = hexToHsb(hex);
  // In dark mode, elevated surfaces are slightly lighter than base
...
function deriveTextTokensDark(hex: string): Record<string, string> {
  // For dark mode text, parse rgba or derive from hex
  if (hex.startsWith('rgba')) {
    // Extract base color and alpha from the input
...
function deriveBorderTokensDark(hex: string): Record<string, string> {
  const { h, s, b } = hexToHsb(hex);
  return {
    'color-border': hex,
...
function buildBgMap(
  ...
  return {
    ...
    // Component backgrounds — base level
    'card-bg': elev,
    'modal-bg': elev,
 ...
 * @param isDark - whether to use dark-mode derivation
 */
export function deriveAllTokens(
  seeds: Record<string, string>,
  isDark = false
): Record<string, string> {
  const result: Record<string, string> = {};

  const derivePrimary = isDark ? derivePrimaryTokensDark : derivePrimaryTokensLight;
  const deriveSemantic = isDark ? deriveSemanticTokensDark : deriveSemanticTokensLight;
  ...
  for (const [key, value] of Object.entries(seeds)) {
    switch (key) {
      case 'color-primary':
        Object.assign(result, derivePrimary(value));
        break;
      case 'color-success':
      case 'color-warning':
      case 'color-danger':
      case 'color-info':
        Object.assign(result, deriveSemantic(key, value));
        break;
      case 'color-bg':
        Object.assign(result, deriveBg(value));
        break;
      case 'color-text':
        Object.assign(result, deriveText(value));
        break;
      case 'color-border':
